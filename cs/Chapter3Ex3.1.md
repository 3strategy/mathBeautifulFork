...
המקבלת מספר תלת-ספרתי ומציגה ומחזירה true אם המספר הוא "מספר משולש", אחרת false.
...
כתבו פונקציה `static int CalcY(int x)` שמקבלת מספר שלם x, ומחזירה את ערך y לפי  
\( y = x^3 + 6x - 10 \)
