...
#### טבלת מעקב מורחבת - מה שתלמידים ירשמו בשלב זה
### טבלת מעקב מקוצרת

| קלט num | i | Math.Pow(i,2) | האם num == Math.Pow(i,2) | s (אחרי) |
|----------|---|---------------|-------------------------|-----------|
| 3        | 1 | 1             | לא                      | 0         |
| 4        | 2 | 4             | כן                      | 4         |
| 5        | 3 | 9             | לא                      | 4         |
...
### טבלת מעקב מורחבת עבור קלטים: 3, 4, 5
...
*הערות:*  
- שורות 2-3: קריאת קלט ועדכון num, עדכון i  
- שורה 4: בדיקת if  
- שורה 5: עדכון s במידה שה-if מתקיים  
- שורה 6: הדפסת num אם נכנס ל-if  
- בסיום (שורה אחרונה): הדפסת הפלט הסופי (Console.WriteLine(s))
...
**למה זה לא עובד?**  
הקוד בודק רק אם הקלט שווה לאחד הריבועים בין 1 ל-25 (רק עבור i=1..5), ולא אם כל קלט הוא ריבוע שלם כלשהו.  
למשל, אם תכניס את המספר 9 בקלט הראשון (כאשר i=1), זה לא יעבור את הבדיקה, למרות ש-9 הוא ריבוע שלם (כי Math.Pow(1,2) = 1).

</details>

{: .subq}
ב. כתבו קוד הקולט 5 מספרים ומדפיס עבור כל אחד אם הוא ריבוע שלם.   (דוגמא: 16 הוא ריבוע שלם, מפני שהמספר 4 (השורש הריבועי שלו) הוא שלם).
