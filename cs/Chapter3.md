...
## השלמות וחזרה
1. **לא הספקנו לדבר על טבלת המעקב**
1. שגיאה נפוצה שראינו: בדוגמא שלהלן המילה Success תודפס תמיד מפני שיש בטעות ; בסוף התנאי. זו שגיאה (לא כל כך נפוצה) אבל מאד מְתַעְתַּעַת מפני שהקוד מתקמפל וקשה לשים לב לטעות. טעות כזו ניתן לגלות בבדיקת מקרים, או, בתשומת לב ל-warnings של VS או, אם ניישר את הקוד באמצעות הקיצורים. שורת ההדפסה לא תהיה מוזחת פנימה וזה אמור להדליק נורה אדומה. 
    ```csharp
    if (num > 5);
...
1. לא ניתן לרשום כמו בפייתון `if (3 < x < 5)`. חייבים `if (x > 3 && x < 5)`
