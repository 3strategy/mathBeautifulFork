...
חלוקת התוכנית לתתי-משימות, קוד לשימוש חוזר ושיפור הקריאות 
{: .box-note}
...
## פונקציות בתכנות
פונקציה (function, פעולה), נקראת לעיתים גם שגרה (procedure) או מתודה (method). זוהי יחידת קוד עצמאית בתוך תוכנית, המבצעת משימה מוגדרת. לפי פרדיגמת התכנות הפרוצדורלי, מומלץ לפרק תוכניות לפונקציות קטנות ככל האפשר, כך שכל פונקציה מבצעת פעולה פשוטה אחת או כמה פעולות קשורות
...
לדוגמה, נגדיר פונקציה סטטית שמדפיסה שורה של כוכביות. הפונקציה לא מקבלת שום פרמטר (הסוגריים ריקים) ולא מחזירה ערך, ולכן סוג הערך המוחזר מוגדר כ-void. 
ניתן לקרוא לפונקציה זו מכל מקום בתוך המחלקה (למשל מתוך Main) על-ידי כתיבת שמה ואחריו סוגריים ריקים:

 <details open markdown="1"><summary>קוד לדוגמה: הגדרת פונקציה ללא פרמטרים וקריאה אליה</summary> {% highlight csharp linenos %}
...
**חשוב:** את הגדרת הפונקציה **יש לכתוב מחוץ לפונקציה Main (ובאותה מחלקה). כלומר, אין לקנן פונקציות זו בתוך זו** (ב־C# 7 יש אפשרות של פונקציות מקומיות, אך לא נעסוק בכך כעת). 
...
- בפונקציות סטטיות נקפיד גם להוסיף את המילה static בהגדרה (כמו בדוגמה), משום שאנו קוראים להן מתוך מתודה סטטית (Main). **בקורס שלנו, לכל פונקציה נוסיף את ה-static. זהו  modifier ונשתמש בו תמיד עד שנלמד עצמים.

### דוגמה 1: הדפסת שורת כוכביות מספר פעמים

נציג כעת פתרון לשימוש חוזר בקוד באמצעות פונקציה. נניח שנרצה להדפיס שלוש שורות של כוכביות (בכל שורה 10 כוכביות). נשווה בין שני מימושים: בלי פונקציה – תוך חזרת קוד, ועם פונקציה.
...
</details> <details markdown="1"><summary>פתרון עם פונקציה</summary>
...
נשכתב את הפונקציה הקודמת שיצרה שורת כוכביות באורך קבוע, כך שתוכל להדפיס שורה באורך גמיש בהתאם לקלט. במקום פונקציה נפרדת לכל אורך, נגדיר פונקציה אחת עם פרמטר שלם הקובע את מספר הכוכביות:
<details markdown="1"><summary>PrintStars – הדפסת שורת כוכביות באורך נתון</summary>
...
Exercise: כתבו פונקציה PrintRectangle המקבלת שני פרמטרים שלמים – rows (שורות) ו-cols (עמודות), ומדפיסה מלבן כוכביות בגודל המבוקש. לדוגמה, עבור קריאה PrintRectangle(3, 5) הפלט יהיה:
*****  
*****  
*****  
