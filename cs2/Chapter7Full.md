...
בקריאה SayHello("Dan") תודפס ההודעה Hello, Dan!. בצורה דומה אפשר לקבל בפרמטרים קלטים מטיפוס double (למספרים ממשיים), char (לתו בודד) וכדומה, או לשלב מספר פרמטרים מסוגים שונים. לדוגמה, פונקציה המקבלת שם וכמות: PrintNameMultiple(string name, int times) תדפיס את השם מספר פעמים לפי הפרמטר השני. 
...
- יתכן שנלמד על העברה לפי הפניה (by ref לא בתכנית) - המאפשרת לפונקציה להשפיע חיצונית. 
