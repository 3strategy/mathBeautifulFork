---
layout: page 
title: "🔢 קשר בין פולינום לגרף" 
subtitle: "מערך שיעור"
mathjax: true
lang: he
---

<!-- https://chatgpt.com/c/682c67eb-7f98-800e-add4-8c055722c75f -->

<details markdown="1">
<summary>מטרות ומדיה</summary>
## 🎯 מטרות השיעור

{: .box-success}
- תלמידים יבינו שפונקציה פולינומית הנתונה כמכפלת גורמים מתאפסת כאשר אחד הגורמים מתאפס.
- יכירו את שיטת "הנחש" ככלי לשרטוט גרף של פולינום לפי שורשיו וסימן הפונקציה.
- ישפרו שפה מתמטית (כתיבת גרירות לנק' אפס, ורישום טבלאי שיעזור בהמשך לטבלאות סיווג קיצון)
- **(בהתאם למה שיוגדר, ולקצב)** ילמדו לפעול גם בכיוון ההפוך: **מגרף לפונקציה**.

---

## מדיה
### רוב השיעור יועבר על הלוח בטוש
### הכנתי Geogebra  להערכתי לא יהיה צורך. אני מכוון לשיעור קצת יותר איטי כדי שיוכלו לעקוב ולרשום.
### המערך מאפשר גם הקרנה על הלוח או למידה עצמית (זה לא התכנון)

## 🧱 שלבי השיעור
</details>

### **1. פתיחה – חזרה קצרה והצגת הנושא**

[מה קורה כשכופלים $$5 \cdot0 = ? \\ 5\cdot0\cdot3 = ?$$](#){: title="שאלה לכיתה - לפתיחת הדיאלוג"}

[איך מציירים את $$?f(x)=(x+1)(x-2)$$](#){: title="שאלה לכיתה, לקישור לחומר קודם (ותמונת מצב הידע בכיתה)" }

ומה אם יש 3 גורמים?
<details markdown="1">
<summary>על מה אפשר להסכים כרגע?</summary>

{: .box-success}
אם פונקציה נתונה בצורה של מכפלת גורמים, לדוגמה:  $$ f(x) = (x+1)(x-2)(x-4) $$  
**אז כל ערך שיגרום לאחד הגורמים להתאפס – יגרום לפונקציה להתאפס**.

</details>
---

### **2. הדגמה: מה קורה כשכופלים באפס?**

#### שאלות:
- מה קורה לביטוי $$ (x+1)(x-2) $$ כאשר $$ x = 2 $$?
- ומה כאשר $$? x = -1 $$
<details markdown="1">
<summary></summary>
  **דגש: $$ x=+2 $$ חיובי כשבסוגריים יש מינוס**

  **שלילי** כשבסוגריים יש **פלוס** $$ (x+1) = 0 ⟹ \boxed{x = -1} $$
</details>

{: .box-warning}
**לסיכום**: הפונקציה מתאפסת בכל מקום בו אחד הגורמים מתאפס.

<details markdown="1">
<summary>הערה למקרה שתלמידים פותחים סוגריים</summary>

{: box-note}
למרות שאנחנו רגילים לפתוח סוגרים - ככל שיעבור הזמן יהיו יותר מקרים בהם נרצה לעשות ההפך. ברוב המקרים נעדיף יצוג של מכפלת גורמים. כנראה שלא אכנס לזה

</details>
---

## **3. שיטת הנחש 🐍 – הסבר ותרגול**
  כיצד נשרטט את הגרף הבא?
  $$ \quad \quad  f(x)  \quad  =  \underset{\downarrow}{(x+1)}\underset{\downarrow}{(x-2)}\underset{\downarrow}{(x-4)} \\
  f(x)=0 ⟹ x=-1, x=2, x=4 $$
<details markdown="1">
<summary>פירוט הצבות</summary>
  $$f(-3) = -70 < 0 ⟹ - \\
  f(0) = 8 > 0 ⟹ + \\
  f(3) = -4 < 0 ⟹ - \\
  f(6) = 56 > 0 ⟹ + $$
</details>
  
  | $$x$$    |  -3 | 0 | 3 | 6  |
  |----------|-----|---|---|----|
  | $$f(x)$$ | -70 | 8 | -4| 56 |
  {: .table-en}


  **נחש**

  |       | $$+$$ |   | $$+$$ |
  |-------|---|---|---|
  |     $$\quad-$$ |   | $$-$$ |   |  
  {: .table-en}

<details markdown="1">
<summary>למורה</summary>
### סיכום מפורט - השלבים בשיטת הנחש:
1. מוצאים את השורשים של הפונקציה (כל ערך שמאפס גורם במכפלה).
2. מחלקים את ציר ה־ $$x$$ לתחומים לפי השורשים (נקודות האפס).
3. מחשבים את סימן הפונקציה בכל תחום ומסמנים **חיוביות מעל** ושליליות **מתחת** לציר. **דגש: מתייחסים ל- $$y$$. מה שמבדיל בין חיובי לשלילי הוא ציר ה-x.**
4. סרטוט נחש (הפולינום) על ידי חיבור הנקודות.

### רישום לוח מוצע:
![רישום לוח](image.png)

</details>
## שלבי שיטת הנחש
1. מוצאים נקודות שמאפסות $$ y=0 $$ 
1. מסמנים על הציר
1. מחשבים סימן (חיוביות / שליליות)
1. משרטטים נחש לפי הסימנים

**מסקנות:**
- הפונקציה מתאפסת ב־ $$ x = -2,\ 1,\ 4 $$
- [**בכל תחום בין השורשים היא שומרת על סימן קבוע. למה?**](#){: title="  אם אף אחד מהביטויים לא משנה סימן, גם הסימן של הביטוי כולו לא משתנה"}

{: .box-warning}
חשוב לוודא שאין תוספת קבוע שמחובר למכפלה ומזיז אותה מעלה. במקרה כזה הכל משתנה. **להדגים על גרף מוזז**

<details markdown="1">
<summary>הרחבה לגבי הזזה אם יש זמן</summary>
**ניתן להדגים שמציאת נקודות התאפסות ללא ההזזה, מאפשרת להסיק מסקנות על הפונקציה המוזזת**
</details>

---

## 🧪 ביחד: תרגול לכיתה

### .שאלה 1. מצאו את שורשי הפונקציות הבאות ושרטטו לפי שיטת הנחש:
**אשאל אותם ונכתוב יחד פונקציות**

{: .subq}
א. $$ f(x) = (x+1)(x-3) $$ 
<details markdown="1">
<summary>אז למה נחש?</summary>
זו פרבולה מחייכת אבל הנחש יותר "חזק"
</details>

{: .subq}
ב. $$ f(x) = x(x-2)(x+4) $$


### שאלה 2. אתגר – שורש כפול
נתונה $$ f(x) = (x+1)^2(x-2) $$

{: .subq}
א. מה קורה ב־ $$\ x = -1 $$?

{: .subq}
ב. כיצד נראית הפונקציה באזור זה?

<details markdown="1">
<summary></summary>
**הפונקציה לא תמיד מזגזגת סביב נקודות ההתאפסות. הנחש מתמודד גם עם המקרים האלו**

{: .subq}
ג. מדוע הגרף "נוגע" בציר במקום לחצות? 
<!-- לא להתעקש לקבל תשובה על זה. -->

</details>



---
## 🧪 תרגול נוסף
<!-- כבר בתירגול מהלוח - זה יקח זמן - תלוי במיומנות שלהם על המחשבון. בוחר מספרים "קלים" עם מספר שלם זמין בין השורשרים -->
1. $$f(x)=(x−2)(x−4)(x−6)$$
1. $$f(x)=(x−2)(4-x)(x−6)$$ 
  היפוך הסדר בין המספרים - למה זה שקול?.
1. $$f(x)=x^3(x−2)$$


<details markdown="1">
<summary>מגרף לאלגברי ועוד</summary>
### שאלה 3 מגרף לבחירת הייצוג האלגברי

{: .box-error}
**על בסיס זמן פנוי. לברר עם א' או א' אם למדו את זה או שגם זה חדש**
### גרסה שאני מעדיף:
כנראה שבמקום הגרף הזה, אצייר על הלוח סקיצה של פולינום ממעלה שלישית עם 3 חיתוכים כגון $$(x+1)(x-1)(x-3)$$ וננסה לבנות ביחד פונצקיה. אחרי שנבנה אות הפולינום, אצייר שיקוף ביחס לציר $$y$$ **ואשאל איך ניתן לקבל צורה כזו**.

### גרסה חלופית -בחירה בין פונקציות
הסתכלו על הגרף הבא:
![נתון הגרף הבא](/assets/img/poly-graph-quiz.png)

הגרף מתאר פולינום המתאפס בשלוש נקודות:  
$$ x = -2,\ x = 0,\ x = 1 $$

שתי פונקציות מוצעות:
- $$ f(x) = x^2(x+2)(x-1) $$
- $$ g(x) = x(x+2)(x-1) $$

### ❓ איזו מבין השתיים מתאימה לגרף?

<details markdown="1"><summary>💡 רמז – נסו להציב ערך כלשהו</summary>

בואו נציב $$ x = -1 $$ בשתי הפונקציות:

{: .table-en}
$$ f(-1) = (-1)^2 \cdot (1) \cdot (-2) = \\
        = 1 \cdot 1 \cdot (-2) = -2 \\
g(-1) = (-1) \cdot (1) \cdot (-2) = 2 $$

בגרף נראה כי בנקודה $$ x = -1 $$ הפונקציה נמצאות **מתחת** לציר ה־x, ולכן ערכה שלילי.  
רק הפונקציה $$ f(x) $$ מתאימה להתנהגות זו.

</details>

---

### 🔍5. נושא מתקדם- נוותר על זה – מעלת הפולינום

{: .box-warning}
רק אם נותר זמן, ניתן לעסוק גם בקשר בין מספר הגורמים למעלת הפולינום:  
- אם כל גורם ממעלה ראשונה, אז מספר הגורמים = מעלת הפולינום. לדוגמה:
  $$ f(x) = (x-2)(x+3)(x-1) \\
   \Rightarrow \text{מעלה 3} $$

---


### 🧠6. (לא מעריך שיהיה זמן. ואני לא רוצה להתפזר) שאלות פתוחות למחשבה

1. האם כל פונקציה פולינומית מתאפסת?
1. האם תמיד ניתן לכתוב פונקציה פולינומית כמכפלת גורמים?
1. מה קורה אם לפולינום אין שורשים ממשיים?
1. כיצד ניתן להסיק על מספר השורשים מתוך צורת הפונקציה?


---
{: .box-warning}
Geogebra ישאר לסוף - רק אם אראה בזה תועלת מעבר ללוח

{: .box-success}
להערכתי הכי נכון בסופו של דבר יהיה **לעבוד ידני לגמרי** על פולינום ממספרים שהתלמידים יגדירו בכל פעם. **less is more.**. כאן אשרטט את הציר, ואסביר את שיטת הנחש.

</details>

<!-- [![Geogebra](/assets/img/GeogebraSnake.png)](https://www.geogebra.org/classic/h379jjtn)
[⬅ מעבר לגאוגברה](https://www.geogebra.org/classic/h379jjtn) -->
<a href="https://www.geogebra.org/classic/h379jjtn" target="_blank">
  <img src="/assets/img/GeogebraSnake.png" alt="Geogebra">
</a>

<a href="https://www.geogebra.org/classic/h379jjtn" target="_blank">
  ⬅ מעבר לגאוגברה
</a>